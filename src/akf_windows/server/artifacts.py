...
from caselib.uco.observable import WindowsPrefetch, WindowsPrefetchFacet, FileFacet, File, Volume, VolumeFacet
...
class WindowsArtifactService(AKFService):
    ...
    def _parse_single_prefetch_file(self, prefetch_path: Path, include_volume: bool = False) -> WindowsPrefetch | None:
        """
        ...
        for volume in prefetch_obj.volumesInformationArray:
            volume_name = volume["Volume Name"].decode("UTF-16", errors="backslashreplace")
            serial_number = volume["Serial Number"]
            
            volume_obj = Volume(
                hasFacet=[VolumeFacet(volumeID=serial_number)]
            )
        ...
        volume = None
        if include_volume:
            volume = list(volume_objs.values())[0] if volume_objs else None
    ...
    def exposed_collect_prefetch_file(self, prefetch_path: Path) -> WindowsPrefetch | None:
        """
